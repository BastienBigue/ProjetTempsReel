...
void initStruct(void) {
	...
	{
		rt_printf("err = %d\n", err);
		rt_printf("Creation du flag impossible : arrêt du programme\n");
		exit(-1);
	}
    ...
    if (err = rt_sem_create(&semConnectionMoniteur, NULL, 0, S_FIFO)) {
        rt_printf("Error semaphore create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    if (err = rt_sem_create(&semConnectionOK, NULL, 0, S_FIFO)) {
        rt_printf("Error semaphore create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&tbattery, NULL, 0, PRIORITY_TBATTERY, 0)) { //task battery
        rt_printf("Error task create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
